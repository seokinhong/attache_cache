    ...
    if(comp_alg == COMP_ALG::BDI) {
        ...
        for (auto &k: base_size) {
            ...
            for (auto &base:data_vec) {
                ...
                output->verbose(CALL_INFO,5,0,"k:%d base: %llx max_delta_size_base: %d max_delta_size_immd: %d num_delta_base:%d num_delta_immd:%d compressed_size:%d compression ratio:%lf\n",
        ...
        output->verbose(CALL_INFO,5,0,"[CompressionResult] k:%d base: %llx compressed_size:%d compression ratio:%lf\n",
