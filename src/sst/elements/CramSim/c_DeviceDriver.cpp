...
c_DeviceDriver::c_DeviceDriver(Component *owner, Params& params) : SubComponent(owner) {
    ...
    m_bankParams[e_BankTiming::nRC] = (uint32_t) params.find<uint32_t>("nRC", 55, l_found);
    if (!l_found) {
        std::cout << "nRC value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nRRD] = (uint32_t) params.find<uint32_t>("nRRD", 4, l_found);
    if (!l_found) {
        std::cout << "nRRD value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nRRD_L] = (uint32_t) params.find<uint32_t>("nRRD_L", 6, l_found);
    if (!l_found) {
        std::cout << "nRRD_L value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nRRD_S] = (uint32_t) params.find<uint32_t>("nRRD_S", 4, l_found);
    if (!l_found) {
        std::cout << "nRRD_S value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nRCD] = (uint32_t) params.find<uint32_t>("nRCD", 16, l_found);
    if (!l_found) {
        std::cout << "nRRD_L value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nCCD] = (uint32_t) params.find<uint32_t>("nCCD", 4, l_found);
    if (!l_found) {
        std::cout << "nCCD value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nCCD_L] = (uint32_t) params.find<uint32_t>("nCCD_L", 5, l_found);
    if (!l_found) {
        std::cout << "nCCD_L value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nCCD_L_WR] = (uint32_t) params.find<uint32_t>("nCCD_L_WR", 1, l_found);
    if (!l_found) {
        std::cout << "nCCD_L_WR value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nCCD_S] = (uint32_t) params.find<uint32_t>("nCCD_S", 4, l_found);
    if (!l_found) {
        std::cout << "nCCD_S value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nAL] = (uint32_t) params.find<uint32_t>("nAL", 15, l_found);
    if (!l_found) {
        std::cout << "nAL value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nCL] = (uint32_t) params.find<uint32_t>("nCL", 16, l_found);
    if (!l_found) {
        std::cout << "nCL value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nCWL] = (uint32_t) params.find<uint32_t>("nCWL", 16, l_found);
    if (!l_found) {
        std::cout << "nCWL value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nWR] = (uint32_t) params.find<uint32_t>("nWR", 16, l_found);
    if (!l_found) {
        std::cout << "nWR value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nWTR] = (uint32_t) params.find<uint32_t>("nWTR", 3, l_found);
    if (!l_found) {
        std::cout << "nWTR value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nWTR_L] = (uint32_t) params.find<uint32_t>("nWTR_L", 9, l_found);
    if (!l_found) {
        std::cout << "nWTR_L value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nWTR_S] = (uint32_t) params.find<uint32_t>("nWTR_S", 3, l_found);
    if (!l_found) {
        std::cout << "nWTR_S value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nRTW] = (uint32_t) params.find<uint32_t>("nRTW", 4, l_found);
    if (!l_found) {
        std::cout << "nRTW value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nEWTR] = (uint32_t) params.find<uint32_t>("nEWTR", 6, l_found);
    if (!l_found) {
        std::cout << "nEWTR value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nERTW] = (uint32_t) params.find<uint32_t>("nERTW", 6, l_found);
    if (!l_found) {
        std::cout << "nERTW value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nEWTW] = (uint32_t) params.find<uint32_t>("nEWTW", 6, l_found);
    if (!l_found) {
        std::cout << "nEWTW value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nERTR] = (uint32_t) params.find<uint32_t>("nERTR", 6, l_found);
    if (!l_found) {
        std::cout << "nERTR value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nRAS] = (uint32_t) params.find<uint32_t>("nRAS", 39, l_found);
    if (!l_found) {
        std::cout << "nRAS value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nRTP] = (uint32_t) params.find<uint32_t>("nRTP", 9, l_found);
    if (!l_found) {
        std::cout << "nRTP value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nRP] = (uint32_t) params.find<uint32_t>("nRP", 16, l_found);
    if (!l_found) {
        std::cout << "nRP value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nRFC] = (uint32_t) params.find<uint32_t>("nRFC", 420, l_found);
    if (!l_found) {
        std::cout << "nRFC value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nREFI] = (uint32_t) params.find<uint32_t>("nREFI", 9360, l_found);
    if (!l_found) {
        std::cout << "nREFI value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nFAW] = (uint32_t) params.find<uint32_t>("nFAW", 16, l_found);
    if (!l_found) {
        std::cout << "nFAW value is missing ... exiting" << std::endl;
        exit(-1);
    }

    m_bankParams[e_BankTiming::nBL] = (uint32_t) params.find<uint32_t>("nBL", 4, l_found);
    if (!l_found) {
	...
	if(k_useRefresh) {
		...
		for (unsigned l_id = 0; l_id < m_numRanks; l_id++) {
			if (m_currentREFICount[l_id] > 0) {
				...
				m_currentREFICount[l_id] = m_bankParams.at(e_BankTiming::nREFI);
			}
				...
				if (sendCommand((l_cmdPtr), l_bank)) {
					...
					if (l_cmdPtr->isColCommand()) {
						assert((((m_lastDataCmdType != ((l_cmdPtr))->getCommandMnemonic()) ||
							   ...
							   (std::min(m_bankParams.at(e_BankTiming::nBL),
										 std::max(m_bankParams.at(e_BankTiming::nCCD_L), m_bankParams.at(e_BankTiming::nCCD_S)))))));
...
void c_DeviceDriver::initACTFAWTracker()
{
	...
	for(int i=0; i<m_numRanks;i++)
	{
		...
		l_cmdACTFAWTracker.resize(m_bankParams.at(e_BankTiming::nFAW)-1, 0);
...
void c_DeviceDriver::initRefresh() {
	...
	for(int i=0;i<m_numRanks;i++)
	{
		...
		m_currentREFICount[i]=m_bankParams.at(e_BankTiming::nREFI)/(i+1);
...
unsigned c_DeviceDriver::getNumIssuedACTinFAW(unsigned x_rankid) {
	...
	assert(m_cmdACTFAWtrackers[x_rankid].size() == m_bankParams.at(e_BankTiming::nFAW)-1);
